...
class ResNetMultiImageInput(models.ResNet):
    ...
    """
# ResNetMultiImageInput 是基于多输入图像的 ResNet 编码器，支持多帧图像输入，适用于深度估计任务。
...
# num_input_images 表示输入帧数，num_input_images * 3 是卷积的输入通道数。
# 该类定义了一个自定义的 conv1 层，以适应多帧输入。每帧图像有 3 个通道（RGB），所以总输入通道数为 num_input_images * 3。
...
# 初始化卷积层权重，卷积使用 kaiming_normal 初始化，批归一化层的权重设为 1，偏置设为 0。
'''
def resnet_multiimage_input(num_layers, pretrained=False, num_input_images=1):
    ...
    """
# 该方法创建一个 ResNetMultiImageInput 实例，并允许使用 ImageNet 预训练权重。
    ...
    return model
# 	•	支持 ResNet-18 和 ResNet-50 两种深度的 ResNet。
# 	•	若使用预训练模型，则将预训练模型的 conv1 权重扩展，以适应多帧输入。
'''
from torchvision.models import resnet18, ResNet18_Weights

def resnet_multiimage_input(num_layers, pretrained, num_input_images=1):
    """Constructs a ResNet model with custom input layers for multi-image input."""
    assert num_layers in [18, 34], "Can only run with Resnet 18 or 34"

    if num_layers == 18:
        model = resnet18(weights=ResNet18_Weights.DEFAULT if pretrained else None)
    else:
        model = resnet34(weights=ResNet18_Weights.DEFAULT if pretrained else None)

    # 修改第一层卷积，适应 num_input_images 个输入通道
    model.conv1 = torch.nn.Conv2d(
        num_input_images * 3, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
...
class ResnetEncoderMatching(nn.Module):
    ...
    """
# ResnetEncoderMatching 类用于包含匹配特征的 ResNet 编码器。它增加了一个基于深度的特征匹配卷积（cost volume）计算方法。
...
# num_layers：指定 ResNet 的层数。
# min_depth_bin 和 max_depth_bin：设置最小和最大深度范围。
# num_depth_bins：深度划分的数量，用于生成 cost volume。
# adaptive_bins：设置为 True 时，深度范围会根据图像的动态调整。
# depth_binning：设置深度划分方式，可以为 linear（线性深度划分）或 inverse（逆深度划分）。
...
# 根据 depth_binning 设置计算深度划分，如果为 inverse，则按逆深度线性分布，否则按线性分布。
...
# 计算 current_feats（当前帧特征）和 lookup_feats（参考帧特征）之间的 cost volume，通过相对姿态 relative_poses 将参考帧特征对齐到当前帧。
# 使用 grid_sample 实现特征的空间变换，最终返回每个批次的 cost volume 以及缺失的掩码 cost_volume_masks。
...
# 这是一个基本的 ResNet 编码器类，适用于单帧图像输入。
    def __init__(self, num_layers, pretrained, num_input_images=1, **kwargs):
        ...
        if num_layers > 34:
            self.num_ch_enc[1:] *= 4
# 支持多帧和单帧图像输入，单帧图像输入直接使用 torchvision 的预训练模型。
...
# 通过每一层 ResNet 层，提取特征并逐层保存至 self.features。