...
class EarlyStopping:
    def __init__(self, patience=3, min_delta=0.01):
        """
        Args:
            patience (int): 连续多少个 epoch 验证损失没有改善时停止训练。
            min_delta (float): 判定改善的最小变化值。
        """
...
class Trainer:
    ...
    def train(self):
        ...
        early_stopping = EarlyStopping(patience=3, min_delta=0.01)  # 设置早停参数
        ...
        for self.epoch in range(self.opt.num_epochs):
            ...
            print(f"Epoch {self.epoch}, Validation Loss: {val_loss:.4f}")
    ...
    def val(self):
        ...
        self.set_eval()
        try:
            inputs = next(self.val_iter)  # 修改为 next(self.val_iter)
        except StopIteration:
            self.val_iter = iter(self.val_loader)
            inputs = next(self.val_iter)  # 修改为 next(self.val_iter)
...
### chang feng
    ...
    def log(self, mode, inputs, outputs, losses):
        ...
        # 记录图像
        for j in range(min(4, self.opt.batch_size)):  # 最多记录四张图像
            s = 0  # 只记录最大的尺度
            for frame_id in self.opt.frame_ids:
                writer.add_image(
                    "color_{}_{}/{}".format(frame_id, s, j),
            ...
            # 处理 disp 和 disp_mono
            disp = outputs[("disp", s)][j, 0].cpu().detach().numpy()

            # 检查 disp 的维度，如果是二维的灰度图像则扩展为 [1, H, W]
            if disp.ndim == 2:
                disp = np.expand_dims(disp, axis=0)  # 扩展维度以符合 [C, H, W] 格式

            writer.add_image("disp_multi_{}/{}".format(s, j), disp, self.step)

            disp_mono = outputs[('mono_disp', s)][j, 0].cpu().detach().numpy()

            # 处理 disp_mono
            if disp_mono.ndim == 2:
                disp_mono = np.expand_dims(disp_mono, axis=0)  # 扩展维度

            writer.add_image("disp_mono/{}".format(j), disp_mono, self.step)

            # lowest_cost 和 consistency_mask
            if outputs.get("lowest_cost") is not None:
                lowest_cost = outputs["lowest_cost"][j].cpu().numpy()

                # 检查 lowest_cost 的维度并扩展为 [1, H, W] 格式
                if lowest_cost.ndim == 2:
                    lowest_cost = np.expand_dims(lowest_cost, axis=0)

                consistency_mask = outputs['consistency_mask'][j].cpu().detach().unsqueeze(0).numpy()

                min_val = np.percentile(lowest_cost, 10)
                max_val = np.percentile(lowest_cost, 90)
                lowest_cost = np.clip(lowest_cost, min_val, max_val)
